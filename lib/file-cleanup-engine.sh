...
get_project_specific_patterns() {
    ...
    case "$project_type" in
        "bash-scripts")
            # 对于bash-scripts项目，清理旧的项目结构目录和不符合GNU标准的文件
            echo "claude-engine scripts templates dotfiles config docs examples project_docs PROJECT_MANAGEMENT_RULES.md memory.sqlite .claude-lang project_types"
            ;;
...
show_scan_results() {
    ...
    if [ "$SCAN_TEMP_COUNT" -gt 0 ]; then
        echo "🗂️ 临时文件 (${SCAN_TEMP_COUNT}个文件, $(get_human_readable_size $SCAN_TEMP_SIZE)):"
    ...
    if [ "$SCAN_BUILD_COUNT" -gt 0 ]; then
        echo "📦 编译产物 (${SCAN_BUILD_COUNT}个文件, $(get_human_readable_size $SCAN_BUILD_SIZE)):"
    ...
    if [ "$SCAN_LOG_COUNT" -gt 0 ]; then
        echo "📄 日志文件 (${SCAN_LOG_COUNT}个文件, $(get_human_readable_size $SCAN_LOG_SIZE)):"
    ...
    if [ "$SCAN_CONFIRM_COUNT" -gt 0 ]; then
        echo "⚠️ 需要确认的文件 (${SCAN_CONFIRM_COUNT}个):"
    ...
    echo "💾 总计: ${total_count}个文件/目录, 预计释放 $(get_human_readable_size $total_size) 空间"
}
...
selective_cleanup() {
    echo ""
    echo "选择性清理模式:"
    echo "1. [✓] 临时文件 (${SCAN_TEMP_COUNT}个)"
    echo "2. [✓] 编译产物 (${SCAN_BUILD_COUNT}个)"
    echo "3. [ ] 日志文件 (${SCAN_LOG_COUNT}个)"
    echo "4. [ ] 需确认文件 (${SCAN_CONFIRM_COUNT}个)"
    echo ""
...
show_cleanup_results() {
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        echo ""
        echo "📋 清理详情:"
        [ "${CLEANUP_TEMP:-true}" = true ] && echo "  • 临时文件: ${SCAN_TEMP_COUNT}个已删除"
        [ "${CLEANUP_BUILD:-true}" = true ] && echo "  • 编译产物: ${SCAN_BUILD_COUNT}个已删除"
        [ "${CLEANUP_LOG:-false}" = true ] && echo "  • 日志文件: ${SCAN_LOG_COUNT}个已删除"
        [ "${CLEANUP_CONFIRM:-false}" = true ] && echo "  • 需确认文件: ${SCAN_CONFIRM_COUNT}个已删除"
        echo ""
